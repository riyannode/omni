...
describe("agent inspection prompt profiles", () => {
  test("homepage quick test is Arc Testnet only", () => {
    ...
    expect(AGENT_QUICK_TEST_PROMPT).toContain("No fallback.");
  ...
  test("payment safety rules remain explicit and compact", () => {
    ...
    expect(AGENT_QUICK_TEST_PROMPT).toContain("HTTP status and the PAYMENT-REQUIRED header");
    ...
    expect(AGENT_QUICK_TEST_PROMPT).toContain("Never expose secrets");
  });
  ...
  test("resource validation is semantic and resolves relative resources against the full request URL", () => {
    ...
    expect(prompt).toContain("full original OMNI request URL, not only its origin");
  ...
  test("copied prompts stay materially shorter", () => {
    ...
    expect(wordCount(buildAgentInspectionPrompt(packageInput))).toBeLessThanOrEqual(170);
  });
});
