...
export interface ThreatIntelStore {
  lookupEndpoint(resource: string, payTo?: string): Promise<{ checked: boolean; findings: ThreatFinding[] }>;
  lookupPackage(ecosystem: string, name: string, version: string): Promise<{ checked: boolean; findings: ThreatFinding[] }>;
  status(): Promise<ThreatIntelStatus>;
}

class NoopThreatIntelStore implements ThreatIntelStore {
  async lookupEndpoint(): Promise<{ checked: boolean; findings: ThreatFinding[] }> { return { checked: false, findings: [] }; }
  async lookupPackage(): Promise<{ checked: boolean; findings: ThreatFinding[] }> { return { checked: false, findings: [] }; }
