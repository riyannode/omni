...
Arc Testnet paid lifecycle: verified historically on the tested OMNI paid path, including Circle Agent Wallet payment, Gateway settlement, durable PostgreSQL persistence, execution, recovery/replay, and Circle transfer reconciliation. Arc Mainnet configuration: prepared / pending live acceptance. The mainnet-ready seller configuration targets `https://gateway-api.circle.com` for both official middleware and transfer lookup; no payment, persistence, or recovery state machine changes are required. Activation is a separate deployment/environment operation. Live mainnet payment, settlement, persistence, replay, reconciliation, broader route-by-route acceptance, multi-chain acceptance, and fleet validation remain separate gates.
