...
### ERC-8004 agent evidence (current)

```text
ERC-8004 IdentityRegistry ─┐
ERC-8004 ReputationRegistry ├─ read-only provider adapters ─┐
registration URI / agent card ─┘                              v
                                                   OmniIntelligence
                                                          ↓
                                         normalized RiskSnapshot / AgentRisk features
                                                          ↓
                                                  deterministic RiskEngine
```

Provider adapters acquire and normalize ERC-8004 identity, policy-eligible reputation, registration, agent-card, and advertised-service evidence. Identity and reputation reads are read-only; scoring remains deterministic in `RiskEngine`, and caller policy remains outside it. The ERC-8004 identity/reputation chain selected by the agent request is independent from the Circle/x402 payment chain used to pay OMNI.

`RiskEngine` is a deep deterministic module. Provider adapters only acquire/normalize evidence. Repository assessment first resolves a GitHub ref to an immutable commit SHA (re-resolved on every request so branch movement is observed), and only the immutable commit identity may enter the assessment cache; the expensive tree/file collection runs once per commit, not per branch lookup. Repository dependency resolution associates each manifest with a proven governing lockfile across NPM/package-lock, npm-shrinkwrap, Bun, pnpm, Yarn classic, Cargo workspaces, exact requirements pins, authoritative uv/Poetry locks, and selected Go module snapshots from `vendor/modules.txt`; bare `go.mod` requirements remain minimum-version evidence and do not become exact coordinates without vendor selection proof. Parser and byte/entry budgets fail closed when association or exact versions cannot be proven. Local workspace/path, VCS, URL, and replacement dependencies are not emitted as registry coordinates. Unsupported formats are detected and reported as limitations rather than silently counted as covered. Per-package deps.dev observations (`dependencyObservations`: coordinate, licenses, advisory IDs, graph status, provenance) and compact repository dependency vulnerability observations (`dependencyVulnerabilities`: exact coordinates, normalized OSV findings, separate `MAL-*` IDs, CISA KEV correlation) are typed fields of `RepositoryEvidence` so a future bounded repository-audit agent consumes them without parsing generic evidence detail. Repository OSV queries map internal `NPM` to `npm`, `CARGO` to `crates.io`, `PYPI` to `PyPI`, and `GO` to `Go`; unresolved coordinates are never queried, and incomplete/deferred scans remain `UNKNOWN`. These repository observations now feed deterministic repository scoring through pre-truncation policy-neutral summaries. Security-practice, dependency vulnerability, explicit KEV, active MAL-*, and licensed threat-intelligence risks aggregate with `MAX`; source failures and incomplete evidence affect coverage and recommendation gating, not observed-risk score. Repository assessments use the versioned `repository-coverage-v1` source-state ledger and `scoreStatus`: source failures affect coverage and recommendation gating, while observed repository risk remains derived only from resolved risk evidence. Package assessments use the versioned `package-coverage-v2` source-state ledger; its existing scoring semantics remain unchanged. Offline evaluation replays current-cohort rows plus historical v1 package/x402/dependency_set rows (their feature extraction is semantically unchanged); historical repository 3/3 and v1 rows remain incompatible because replaying them would reinterpret historical evidence under the new repository fields; supported 3/3 and v1 package/x402/dependency_set rows remain replayable. `HistoryStore` owns x402 state-change history. `ThreatIntelStore` owns licensed IOC lookup and is vendor-neutral so commercial data suppliers can be replaced without changing the domain model.
...
the Arc mainnet target is `https://gateway-api.circle.com` for both official
middleware and transfer lookup, with no payment, persistence, or recovery
state machine changes required. Remaining acceptance gates include broader
route coverage, multi-chain acceptance, fleet validation, and capacity testing.

Historical note: Arc Testnet paid lifecycle was verified earlier on the tested
OMNI paid path, including Circle Agent Wallet payment, Gateway settlement,
durable PostgreSQL persistence, execution, recovery/replay, and Circle transfer
reconciliation. Historical Testnet evidence is not mainnet evidence.
