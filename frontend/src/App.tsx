...
function getDocsArticle(articleId: DocsArticleId): DocsArticle {
  ...
  const articles: Record<Exclude<DocsArticleId, "package" | "repository" | "dependencies" | "preflight">, DocsArticle> = {
    ...
    results: {
      ...
      sections: [
        { id: "assessment-fields", title: "The assessment fields.", content: <dl className="docs-field-list"><div><dt><code>riskScore</code></dt><dd>0 to 100. Higher means more observed decision risk. For repositories, source failures and coverage loss do not increase the observed score; package and x402 scoring remain subject-specific.</dd></div><div><dt><code>scoreStatus</code></dt><dd><code>measured</code>, <code>measured_partial</code>, or <code>insufficient_evidence</code>. It exposes evidence sufficiency, not statistical confidence.</dd></div><div><dt><code>recommendation</code></dt><dd>Advisory next action from deterministic policy using observed risk and, where applicable, evidence sufficiency. Your caller policy remains in control.</dd></div><div><dt><code>evidenceCoverage</code></dt><dd>0 to 1. It shows how much applicable subject-specific evidence resolved, not the probability that a recommendation is correct.</dd></div><div><dt><code>freshness</code></dt><dd>When an expiry is present, obtain new evidence before you act after that time.</dd></div></dl> },
    ...
    evidence: {
      ...
      sections: [
        ...
        { id: "response-formats", title: "Use the format your client needs.", content: <div className="docs-reader__note"><strong>JSON is the default response.</strong><p>JSON is compact: raw evidence, provider payloads, and Markdown artifacts are omitted. Send <code>Accept: text/markdown</code> when you need only the human-readable version. Errors remain JSON.</p></div> },
      ],
    },
    payment: {
      ...
      sections: [
        ...
        { id: "challenge", title: "2. Read the exact 402 challenge.", content: <p>Ask the protected resource without paying. Compare the resource, USDC amount, network, and scheme in <code>PAYMENT-REQUIRED</code> with the preflight and your own policy. For payment to OMNI, select an acceptable Circle-supported MAINNET option actually offered by the live challenge. No testnet use or fallback; STOP if none is acceptable.</p> },
    ...
    wallet: {
      ...
      intro: "This guide covers mainnet buyer integration with Circle CLI. Production facilitator selection is controlled at runtime by CIRCLE_FACILITATOR_URL; the Arc mainnet target is https://gateway-api.circle.com. Arc Mainnet paid lifecycle: verified on September 17, 2026 on the tested GET /v1/package/risk route with a real 5000-atomic / 0.005 USDC x402 payment on Arc Mainnet (eip155:5042), using Circle Agent Wallet + Circle Gateway, HTTP 200, durable persistence, reconciliation, and replay without duplicate settlement.",
      sections: [
        ...
        { id: "login-mainnet", title: "Use a mainnet Agent Wallet.", content: <><p>Use/reuse the official Circle Agent Wallet. Follow <a href="https://agents.circle.com/skills/setup.md" target="_blank" rel="noreferrer">Circle’s setup/login guide</a>. Mainnet and testnet sessions are separate. Never put OTPs or session data in the repository.</p><p>General API / COPY AGENT PROMPT selects any acceptable Circle-supported MAINNET option actually offered by PAYMENT-REQUIRED, not a static allowlist. No testnet use or fallback. The wallet must support the selected network; otherwise STOP.</p></> },
        { id: "wallet-readiness", title: "Check readiness before authorizing payment.", content: <><p>STOP if the selected mainnet wallet is not payment-ready or its Gateway balance cannot cover the exact USDC payment. This guide does not select a buyer address or authorize funding or deposits.</p><p>TRY WITH YOUR AGENT is separately Arc-specific: ARC MAINNET ONLY, eip155:5042, Circle CLI chain ARC. If absent from PAYMENT-REQUIRED, STOP. No other network or fallback.</p></> },
        { id: "inspect-and-pay", title: "Read the live challenge before payment.", content: <><p>Send the exact OMNI request unpaid first with <code>Accept: application/json</code>. Read PAYMENT-REQUIRED on HTTP 402; an empty JSON body is valid. Validate the offered network, scheme, recipient, and exact USDC amount: the package endpoint is 5000 atomic units / 0.005000 USDC.</p><p>Resolve the challenge resource against the original URL and require the same HTTPS origin, path, and query names/values, with no missing or extra keys. Otherwise STOP. Use a fresh UUID v4 <code>Idempotency-Key</code> per logical request; an allowed retry reuses the same request and key. Authorize at most one payment; uncertain validation, funds, or payment state requires STOP.</p><p>Use the successful OMNI JSON as the report source; unwrap <code>data.response</code> if Circle CLI returns an envelope. Return a concise human-readable risk report. Do not make another paid request or request another representation.</p><p>Report only facts present in OMNI JSON or directly observed during payment. Do not infer omitted details or map riskScore to a severity. OMNI dimension values are risk levels, not quality ratings.</p></> },
